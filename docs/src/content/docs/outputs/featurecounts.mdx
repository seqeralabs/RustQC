...
All featureCounts output files use the BAM file stem as a prefix. Each output can be individually enabled or disabled via the [configuration file](/usage/configuration/).
...
Use a [YAML configuration file](/usage/configuration/) to control which featureCounts outputs are generated. All outputs are enabled by default. Set any to `false` to skip generation:
