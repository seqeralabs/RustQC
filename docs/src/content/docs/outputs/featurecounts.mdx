...
description: How RustQC produces featureCounts-compatible read count files, assignment summaries, and biotype count outputs in a single pass alongside dupRadar analysis.
---
...
RustQC performs gene-level read counting in the **same pass** as the dupRadar duplication rate analysis. During BAM processing, RustQC simultaneously:
...
This single-pass approach eliminates the need for a separate featureCounts run, which is required in the traditional R dupRadar workflow. The featureCounts output is produced at zero additional runtime cost.
...
All featureCounts output files use the BAM file stem as a prefix. Each output can be individually enabled or disabled via the [configuration file](/RustQC/usage/configuration/).
...
Use a [YAML configuration file](/RustQC/usage/configuration/) to control which featureCounts outputs are generated. All outputs are enabled by default. Set any to `false` to skip generation:
...
The primary difference is architectural: RustQC performs counting and dupRadar analysis in a single pass over the BAM file, while the traditional workflow requires running featureCounts as a separate step before dupRadar can process the counts. This means the featureCounts output comes at no additional runtime cost when running RustQC.
