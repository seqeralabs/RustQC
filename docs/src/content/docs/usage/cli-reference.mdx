...
Skip the validation that checks for duplicate-flagged reads in the BAM file.
By default, RustQC verifies that reads have the duplicate flag (SAM flag `0x400`)
set during parsing and exits with an error if none are found.

Use this flag if you want to run on a file without duplicate marking for
testing purposes.
...
`featurecounts/`, `rseqc/<tool>/`, `qualimap/`, `bigwig/`, `preseq/`, and `samtools/`
