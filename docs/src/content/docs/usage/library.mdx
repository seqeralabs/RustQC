...
SAM/BAM/CRAM I/O is handled by the pure-Rust [noodles](https://crates.io/crates/noodles)
crate. A small C++ component (used by the preseq tool) is built from source, so a
working C++ compiler (`g++` or `clang++`) is required when building. No htslib or
cmake dependencies are needed. No runtime dependencies are added beyond what the
binary already needs.
