...
Genes with zero reads have a duplication rate of `0` and RPK/RPKM values of `0`. The format and values are identical to R dupRadar's `_dupMatrix.txt` output, including RPKM calculations which now match upstream exactly.
