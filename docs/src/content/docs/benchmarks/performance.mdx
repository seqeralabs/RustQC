...
RustQC speed is typically bound by CPU and disk I/O. It is designed to heavily
parallelise work to achieve the best performance possible with multiple CPUs.
...
works well. RustQC compiles to x86 CPU archiecture or ARM, so the cheaper and
more energy-efficient Graviton instances perform well.
:::
