...
processing. This page compares the output of each tool against the originals on a
10 GB paired-end BAM file (GM12878, ENCODE).
...
RustQC produces the full `samtools stats` output, including the SN (Summary
Numbers) section and all histogram sections (FFQ, LFQ, GCF, GCL, GCC, GCT,
FBC, LBC, FTC, LTC, IS, RL, FRL, LRL, MAPQ, ID, IC, COV, CHK). The SN
section is the portion parsed by MultiQC for key alignment statistics.
