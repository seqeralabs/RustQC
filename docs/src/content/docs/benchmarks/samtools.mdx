...
validated on two datasets from an AWS cloud run (nf-core AWS megatests, 2026-03-07).

## Performance

All samtools-compatible outputs are produced together in the same single-pass BAM
reading that also covers dupRadar, featureCounts, RSeQC (including TIN), preseq, and gene body
coverage. The `RUSTQC_RNA` times below reflect the full all-in-one run; individual
samtools tool times are shown for reference.
...
| Tool                        |     Runtime |         RSS |
| --------------------------- | ----------: | ----------: |
| samtools flagstat           |       2m 8s |      5.2 MB |
| samtools idxstats           |         19s |      4.8 MB |
| samtools stats              |      4m 44s |      8.4 MB |
| **RUSTQC_RNA (all-in-one)** | **18m 52s** | **13.8 GB** |

`RUSTQC_RNA` runs **all** QC tools in a single BAM pass, so its time is not
directly comparable to any individual samtools command — it replaces the combined
runtime of samtools flagstat + idxstats + stats + RSeQC + Qualimap + dupRadar +
featureCounts + preseq.
...
- **Platform**: AWS cloud (nf-core AWS megatests), 2026-03-07
