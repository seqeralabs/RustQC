...
<Aside type="tip" title="Citing RustQC">
If you use RustQC, please cite the original tools.
RustQC writes a `CITATIONS.md` file to the output directory with each run,
...
| Library | Purpose |
|---------|---------|
| [noodles](https://crates.io/crates/noodles) | SAM/BAM/CRAM file I/O (pure Rust) |
...
<div style={{textAlign: "center", margin: "2rem 0 3rem"}}>
  <a href="https://seqera.io" target="_blank" style={{display: "inline-block"}} rel="noopener noreferrer">
    <img className="only-light" src={`${import.meta.env.BASE_URL}seqera-logo.svg`} alt="Seqera" style={{height: "8rem", width: "auto"}} data-zoom-off />
    <img className="only-dark" src={`${import.meta.env.BASE_URL}seqera-logo-dark.svg`} alt="Seqera" style={{height: "8rem", width: "auto"}} data-zoom-off />
  </a>
</div>
