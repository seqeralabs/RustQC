...
Building from source requires the Rust toolchain, a C++ compiler, and
libfontconfig (for plot rendering). SAM/BAM/CRAM I/O is handled by the pure-Rust
[noodles](https://crates.io/crates/noodles) crate — no htslib or cmake required.
The C++ compiler is used by RustQC's preseq FFI shim (a small C++ wrapper that
links against the host's `std::mt19937` and `std::binomial_distribution` for exact
bootstrap compatibility with upstream preseq).
...
<Tabs>
  ...
  <TabItem label="Ubuntu / Debian" icon="linux">
    ```bash
    # Install system dependencies
    sudo apt install g++ libfontconfig1-dev pkg-config
  ...
  <TabItem label="Fedora / RHEL" icon="linux">
    ```bash
    # Install system dependencies
    sudo dnf install gcc-c++ fontconfig-devel pkg-config
...
The system dependencies are: a C++ compiler (`g++` or `clang++`) and libfontconfig.
