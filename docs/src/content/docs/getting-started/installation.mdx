...
<Tabs>
  ...
  <TabItem label="Linux x86_64 (AVX2)" icon="linux">
    ```bash
    curl -fsSL https://github.com/seqeralabs/RustQC/releases/latest/download/rustqc-linux-x86_64-v3.tar.gz | tar xz --strip-components=1
    chmod +x ./rustqc
    sudo mv ./rustqc /usr/local/bin/
    rustqc --version
    ```
  </TabItem>
  <TabItem label="Linux x86_64 (AVX-512)" icon="linux">
    ```bash
    curl -fsSL https://github.com/seqeralabs/RustQC/releases/latest/download/rustqc-linux-x86_64-v4.tar.gz | tar xz --strip-components=1
    chmod +x ./rustqc
    sudo mv ./rustqc /usr/local/bin/
    rustqc --version
    ```
  ...
  <TabItem label="Linux aarch64 (SVE)" icon="linux">
    ```bash
    curl -fsSL https://github.com/seqeralabs/RustQC/releases/latest/download/rustqc-linux-aarch64-neoverse-v1.tar.gz | tar xz --strip-components=1
    chmod +x ./rustqc
    sudo mv ./rustqc /usr/local/bin/
    rustqc --version
    ```
  ...
  <TabItem label="macOS Apple Silicon (M1)" icon="apple">
    ```bash
    curl -fsSL https://github.com/seqeralabs/RustQC/releases/latest/download/rustqc-macos-aarch64-apple-m1.tar.gz | tar xz --strip-components=1
    chmod +x ./rustqc
    sudo mv ./rustqc /usr/local/bin/
    rustqc --version
    ```
...
:::tip[SIMD-optimized builds]
In addition to the baseline builds, RustQC provides optimized binaries that leverage SIMD instructions for better performance:

- **x86_64**: Baseline (works everywhere) &rarr; **AVX2 / x86-64-v3** (Haswell/Zen 1+, recommended for most modern servers) &rarr; **AVX-512 / x86-64-v4** (Skylake-X/Zen 4+, fastest)
- **aarch64 Linux**: Baseline (NEON, works everywhere) &rarr; **SVE / neoverse-v1** (AWS Graviton 3+, Google Axion, NVIDIA Grace)
- **macOS**: Baseline (all Apple Silicon) &rarr; **Apple M1** (optimized scheduling for M-series chips)
