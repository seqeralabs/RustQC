...
docker run -v $(pwd):/data ghcr.io/seqeralabs/rustqc:latest \
  rna /data/sample.bam --gtf /data/genes.gtf -p -o /data/output/
```
...
- **GTF annotation file** (`--gtf`, required) — provides all annotation needed for every tool. Gene models are derived internally so all analyses run: dupRadar, featureCounts, Qualimap gene body coverage, all 8 RSeQC tools including TIN, preseq, and samtools outputs.
